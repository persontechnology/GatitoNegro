...
export default function InicioMisReserva({navigation}) {
  ...
  const getListadoReservas = async () => {
    try {
      const response = await fetch(API_URL+'listar-mis-reservas',{
        method: 'POST',
        headers: {
  ...
  const confirmarReserva = async (estado) => {
    setLoading(true);
    try {
        const response = await fetch(API_URL+'reservar-eliminar',{
          method: 'POST',
          headers: {
              'Accept': 'application/json',
              'Content-Type': 'application/json',
  ...
  return (
    ...
    <ScrollView refreshControl={
      ...
      <Center mt="2" mb="2">
        <Heading fontSize="xl">LISTADO DE MIS RESERVAS</Heading>
      </Center>
                ...
                <Box alignItems="center">
                  <Pressable maxW="96" onPress={()=>{
                    if(servicio.estado=='SOLICITADO'){
                        abrirAction(servicio.id)
                    }else{
                        toast.show({'description':'SOLO LAS RESERVAS SOLICITADAS PUEDE ELIMINAR'})
                    }
                  }} >
                    {({
                    isHovered,
                    ...
                    return <Box bg={isPressed ? "coolGray.200" : isHovered ? "coolGray.200" : "coolGray.100"} style={{
                      transform: [{
                        ...
                        <HStack alignItems="center" mb="2">
                          ...
                          <Badge colorScheme={servicio.estado=='SOLICITADO'?'info':servicio.estado=='RESERVADO'?'green':'orange'} variant={'solid'} mb="2">{servicio.estado}</Badge>
                        </Box>;
                  }}
                  </Pressable>
                  <Example />
